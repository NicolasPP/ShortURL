...
def test_register_surl_success(uow: UnitOfWork) -> None:
...
def test_register_surl_failure_surl_generation_failed(uow: UnitOfWork) -> None:
    with uow.transaction() as api:
...
def test_get_surl_success(uow: UnitOfWork) -> None:
    with uow.transaction() as api:
        add_user: Result[User] = api.users.add(TEST_EMAIL)
        assert not add_user.failed, "Expected add_user to succeed"
